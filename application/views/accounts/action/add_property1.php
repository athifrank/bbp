<?php
	...
	if (empty($tit) || empty($loc) || empty($lmark) || empty($zone) || empty($area)|| empty($price) || empty($aa) || empty($ptype) || empty($pfrom) || empty($bank) || empty($nc) || empty($br))
	{
		...
		$broom=base64_encode($broom);
		$hall=base64_encode($hall);
		$troom=base64_encode($troom);
		$park=base64_encode($park);
		$floor=base64_encode($floor);
		$amen=base64_encode($amen);
		$rmark=base64_encode($rmark);
		$face=base64_encode($face);
		$l1=base64_encode($l1);
		$l2=base64_encode($l2);*/
		header ("Location: ../add_property1.php?a=2i5&1=$tit&2=$loc&3=$lmark&4=$zone&5=$area&6=$price&7=$aa&8=$ptype&9=$pfrom&10=$bank&11=$nc&12=$br&13=$bhk
		");
	}
	else
	{
		...
		if($query) header ("Location: ../add_image1.php");
		else header ("Location: ../add_property1.php?a=qne");
		$run->db_close();
	}
?>
