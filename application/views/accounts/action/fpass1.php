<?php
	...
	if(empty($mail))
	{
		header ("Location: ../fpass1.php?a=8i5");
	}
	else
	{
				$run->db_open();
				$query=mysql_query("SELECT COUNT(mail),name,use FROM admin WHERE mail LIKE '$mail'");
				$run->db_close();
				...
				$name=$row['name'];
				$eid=$row['use'];
				if($check == 0) {header ("Location: ../fpass1.php?a=8i2");}				
				else
				{
						$key=$run->random_key();
						$key1=strrev(md5($key));
...
<p style="margin-top:30px;">Regards ,<br />Bangalorebestproperty.<br /><a href="http://www.bangalorebestproperty.com/">www.bangalorebestproperty.com</a>
						...
						$sent=mail($to, $subject, $message, $headers);
							...
							{
									$run->db_open();
									$query=mysql_query("UPDATE admin SET pass='$key1' where use=$id");
									$run->db_close();
									header ("Location: ../index1.php?a=i87");
							}
							else
							{
									header ("Location: ../fpass1.php?a=i86");
							}
				}
	}
?>